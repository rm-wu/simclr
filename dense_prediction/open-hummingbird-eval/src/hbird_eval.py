...
class HbirdEvaluation():
    def __init__(self, feature_extractor, train_loader, n_neighbours, augmentation_epoch, num_classes, device, nn_params=None, memory_size=None, dataset_size=None, f_mem_p=None, l_mem_p=None):
        ...
        if self.memory_size is not None:
            ...
            self.num_sampled_features = n_patches // (self.memory_size * self.augmentation_epoch)
            # self.num_sampled_features = self.memory_size // (dataset_size * self.augmentation_epoch)
            ## create memory of specific size
            self.total_mem_size = self.num_sampled_features * dataset_size * self.augmentation_epoch
            self.feature_memory = torch.zeros((self.total_mem_size, self.feature_extractor.d_model))
...
def hbird_evaluation(model, d_model, patch_size, dataset_name:str, data_dir:str, out_dir: str, batch_size=64, input_size=224, 
    ...
    evaluator = HbirdEvaluation(feature_extractor, train_loader, n_neighbours=n_neighbours, 
                        ...
                        dataset_size=dataset_size, f_mem_p=f_mem_p, l_mem_p=l_mem_p,)
