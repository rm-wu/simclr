...
for i,video_name in enumerate(VIDEO_NAMES):
    ...
    print(video_name)
    if video_name.endswith('.mp4') and video_name[:-4] not in ' '.join(os.listdir('videos_mae')):
        ...
        if frames_ is not None:
            ...
            try:
                ...
                compute_cos_sims_per_objects(video, model)
                visualize_traj(video)
                print(video.S.mean())
                torch.save([video.frames, video.seg_maps, video.masks_per_object, video.embeddings], f'videos_mae/{video.name}.pt')
                del video
