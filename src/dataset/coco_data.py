...
from src.transforms.image_transformations import Compose
...
class CocoDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if self.mask_type is None:
            self.coco_train = UnlabelledCoco(
                self.file_list,
                self.train_transforms,
                os.path.join(self.data_dir, "train2017"),
            )
            self.coco_val = UnlabelledCoco(
                self.file_list[: self.size_val_set * self.batch_size],
                self.val_transforms,
                os.path.join(self.data_dir, "val2017"),
            )
        else:
            self.coco_train = COCOSegmentation(
                self.data_dir,
                self.file_list,
                self.mask_type,
                image_set="train",
            ...
            self.coco_val = COCOSegmentation(
                self.data_dir,
                self.file_list_val,
                self.mask_type,
                image_set="val",
...
class COCOSegmentation(Dataset):
    def __init__(
        ...
        with open(os.path.join(root, json_file)) as f:
            ...
            if mask_type == "thing":
                all_thing_cat_sup = set(
                    cat_dict["supercategory"]
                    for cat_dict in all_cat
                    if cat_dict["isthing"] == 1
                )
                super_cat_to_id = {
                    super_cat: i
                    for i, super_cat in enumerate(sorted(all_thing_cat_sup))
                }
                self.cat_id_map = {}
                for cat_dict in all_cat:
                    if cat_dict["isthing"] == 1:
                        self.cat_id_map[cat_dict["id"]] = super_cat_to_id[
                            cat_dict["supercategory"]
                        ]
                ...
                super_cats.remove(
                    "other"
                )  # remove others from prediction targets as this is not semantic
                super_cat_to_id = {
                    super_cat: i for i, super_cat in enumerate(sorted(super_cats))
                }
                super_cat_to_id["other"] = 255  # ignore_index for CE
                self.cat_id_map = {
                    cat_dict["id"]: super_cat_to_id[cat_dict["supercategory"]]
                    for cat_dict in all_cat
                }
        ...
        #     "/home/mereur1/projects/ocl/ssl_nat_aug/data/coco_stuff164k", seg_folder
        # )
        image_dir = os.path.join(root, "images", f"{image_set}2017")
        if not os.path.isdir(seg_dir) or not os.path.isdir(image_dir):
            raise RuntimeError(
                f"Dataset not found or corrupted."
                f"The provided image path is {image_dir} and "
                f"the segmentation path is {seg_dir}"
            )
        # self.images = [os.path.join(image_dir, str(x)) for x in self.file_names]
        self.images = self.file_names
        self.masks = [
            os.path.join(seg_dir, str(x.name).replace("jpg", "png"))
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        if Path(self.masks[index]).exists():
            mask = Image.open(self.masks[index])
        else:
            print(f"{self.masks[index]}")
        ...
        if self.mask_type == "stuff":
            ...
            for cat_id in torch.unique(mask):
                ...
                if cid in self.cat_id_map and cid <= 200:
                    merged_mask[mask == cat_id] = self.cat_id_map[
                        cid
                    ]  # [0, 11] + {255}
                else:
