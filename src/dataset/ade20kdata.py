...
class Ade20kDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        self.val = ADE20K(
            self.root, self.val_transforms, split="val", file_set=self.val_file_set
        )
        self.train = ADE20K(
            self.root,
...
class ADE20K(Dataset):
    ...
    def __init__(
        self, root, transforms, split="train", skip_other_class=False, file_set=None
    ):
        super().__init__()
    ...
    def collect_data(self):
        ...
        annotation_dir = os.path.join(
            self.root, f"annotations/{self.split_to_dir[self.split]}"
        )
        ...
        if self.file_set is None:
            ...
            image_paths = [
                os.path.join(image_dir, f"{f}.jpg") for f in sorted(self.file_set)
            ]
            annotation_paths = [
                os.path.join(annotation_dir, f"{f}.png") for f in sorted(self.file_set)
            ]
