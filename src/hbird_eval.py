...
class HbirdEvaluation:
    def __init__(
        ...
        if self.memory_size is not None:
            ...
            self.num_sampled_features = n_patches // (
                self.memory_size * self.augmentation_epoch
            )
            # self.num_sampled_features = self.memory_size // (dataset_size * self.augmentation_epoch)
            ## create memory of specific size
            self.total_mem_size = (
                self.num_sampled_features * dataset_size * self.augmentation_epoch
            )
            self.feature_memory = torch.zeros(
                (self.total_mem_size, self.feature_extractor.d_model)
            )
            self.label_memory = torch.zeros((self.total_mem_size, self.num_classes))
        ...
        if use_faiss:
            self.use_faiss = True
            self.create_NN_faiss(self.n_neighbours, **nn_params)
        else:
    ...
    def create_NN(
        ...
        self.NN_algorithm = (
            ...
            .reorder(num_reordering_candidates)
            .build()
        )
        ...
        with torch.no_grad():
            for j in tqdm(range(self.augmentation_epoch), desc="Augmentation loop"):
                for i, (x, y) in enumerate(
                    tqdm(train_loader, desc="Memory Creation loop")
                ):
                    x = x.to(self.device)
                    y = y.to(self.device)
                    y = (y * 255).long()
                    y[y == 255] = 0
                    features, _ = self.feature_extractor.forward_features(
                        x
                    )  # features of shape (BS, PS, D)
                    ...
                    if self.memory_size is None:
                        ...
                        sampled_features, sampled_indices = self.sample_features(
                            features, patchified_gts
                        )
                        ...
                        label_hat = label.gather(
                            1,
                            sampled_indices.unsqueeze(-1).repeat(1, 1, label.shape[-1]),
                        )
    ...
    def patchify_gt(self, gt, patch_size):
        ...
        gt = gt.reshape(
            bs, h // patch_size, w // patch_size, c * patch_size * patch_size
        )
        return gt
    ...
    def find_nearest_key_to_query_faiss(self, q):
        ...
        # Retrieve key labels from memory
        key_labels = self.label_memory[neighbors]
        key_labels = key_labels.reshape(bs, num_patches, self.n_neighbours, -1)
        return key_features, key_labels
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for i, (x, y) in enumerate(tqdm(val_loader, desc="Evaluation loop")):
                ...
                if self.use_faiss:
                    key_features, key_labels = self.find_nearest_key_to_query_faiss(q)
                else:
            ...
            if return_knn_details:
                ...
                return jac, {
                    "knns": knns,
                    "knns_labels": knns_labels,
                    "knns_ca_labels": knns_ca_labels,
                }
            else:
                return jac
...
def hbird_evaluation(
    ...
    train_transforms = CombTransforms(
        img_transform=train_transforms_dict["img"],
        tgt_transform=None,
        img_tgt_transform=train_transforms_dict["shared"],
    )
    val_transforms = CombTransforms(
        img_transform=val_transforms_dict["img"],
        tgt_transform=None,
        img_tgt_transform=val_transforms_dict["shared"],
    )
    ...
    f_mem_p = None  # os.path.join(out_dir, "feature_memory.pt")
    l_mem_p = None  # os.path.join(out_dir, "label_memory.pt")
