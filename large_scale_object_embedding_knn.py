...
for MODEL_NAME in ['CLIP', 'DINOv2-reg', 'MAE']:
    ...
    for s in range(0, N, S):
        ...
        for i in range(0,N,S):
            slice__ = min(N-i,S)
            sims = (embeddings[i:i+slice__].unsqueeze(0) * inputs.unsqueeze(1)).sum(-1) # slice_,slice__
            similarity.append(sims.T) # slice__,slice_
        similarity = torch.cat(similarity) # S,slice_
        similarity[s+torch.arange(slice_),torch.arange(slice_)] = -1
