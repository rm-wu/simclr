...
for img_idx in range(num_img):
    ...
    if 'labels' in target:
        ...
        # print('All objects before resize (Source)', [categ_map[l.item()] for l in target['labels']])
        obj_patch_coords = []
        for i in range(len(target['labels'])):
            ...
            if obj_mask.any():
                ...
                obj_str = categ_map[target['labels'][i].item()]
                ...
                obj_patch_coords.append((obj_row, obj_col))
        ...
        z_objects = torch.stack([z[img_idx, :, obj_row, obj_col] for obj_row, obj_col in obj_patch_coords])
        # print(z_objects.shape)
        attn_inter = torch.einsum("oc,ncij->onij", F.normalize(z_objects, dim=1), F.normalize(z, dim=1))
        ...
        attn_inter -= attn_inter.mean([2, 3], keepdims=True)
        attn_inter = attn_inter.clamp(0)

        obj_patch_similarities = attn_inter
        img_acc = 0.0

        for i, (obj_label, obj_str) in enumerate(zip(obj_labels, obj_strs)):
            obj_row, obj_col = obj_patch_coords[i]
            topk_tensor = torch.topk(obj_patch_similarities[i].flatten(), 1, sorted=True)
            # print(topk_tensor.values)
            indices = torch.unravel_index(topk_tensor.indices, obj_patch_similarities.shape[1:])
            ...
            if 'labels' in target_target:
                ...
                for j in range(len(target_target['labels'])):
                    obj_mask = target_target['masks'][j]
                    obj_mask = resize_transform(obj_mask.unsqueeze(0).float()).repeat(3, 1, 1)
                    obj_mask_patches = obj_mask.unsqueeze(0).unfold(2, 14, 14).unfold(3, 14, 14).squeeze(0)
                    obj_mask_patches = obj_mask_patches.squeeze(0).permute(1, 2, 0, 3, 4)
                    ...
                    if obj_exists_at_target:
                        obj_labels_target.append(target_target['labels'][j].item())
                        obj_strs_target.append(categ_map[target_target['labels'][j].item()])
